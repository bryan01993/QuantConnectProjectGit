...
def main():
    ...
    push_project_to_cloud(cmd_vars, project_root)
...
def push_project_to_cloud(cmd_vars, project_root):
    # Construct the correct project path based on the project root
    project_path = os.path.join(project_root, cmd_vars['cmd_algo_name'])

    # Check if the project directory exists
    if not os.path.exists(project_path):
        logging.error(f"Project directory not found at path: {project_path}")
        return

    # Format the command to push the project to cloud
    push_command = f"lean cloud push --project {project_path}"
    try:
