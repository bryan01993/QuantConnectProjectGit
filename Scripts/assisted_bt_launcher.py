...
def call_chatgpt_on_error(error_message, retries=3, delay=5):
    """
    Calls ChatGPT API to explain an error message with retry logic.
    """
    prompt = f"I received the following error while running a Python subprocess:\n\n{error_message}\n\nCan you explain what went wrong and how to fix it?"
    ...
    for attempt in range(retries):
        try:
            ...
            logging.info(f"ChatGPT explanation:\n{explanation}")
            break  # Stop retrying after success
        except openai.error.OpenAIError as e:
            logging.warning(f"ChatGPT call failed (attempt {attempt+1}): {e}")
            if attempt < retries - 1:
                time.sleep(delay)
            else:
                logging.error("Failed to get explanation from ChatGPT after multiple attempts.")
