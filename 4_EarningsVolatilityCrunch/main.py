...
class EarningsVolatilityCrunch(QCAlgorithm):
    ...
    class PosState:
        def __init__(self):
            ...
            self.min_exit_time = None  # do not exit before this
            self.key = None  # (u, near[, strike])
    ...
    @monitor_execution
    def OnEndOfDay(self):
        # collect all invested holdings with their portfolio percent
        holdings = [
            (kvp.Key, kvp.Value.HoldingsValue / self.Portfolio.TotalPortfolioValue)
            for kvp in self.Portfolio.items() if kvp.Value.Invested
        ]
        if not holdings:
            return


        # sort by descending % and take top 5
        top5 = sorted(holdings, key=lambda x: x[1], reverse=True)[:5]

        for sym, pct in top5:
            sec_type = sym.ID.SecurityType  # Equity, Option, etc.
    ...
    # @monitor_execution
    def get_two_closest_option_chains(self, symbol: Symbol, earnings_time: datetime) -> Tuple[List[Symbol], List[Symbol]]:
        """
    ...
    def OnData(self, data: Slice):
        ...
        for kvp in self.Portfolio:
            ...
            # Only equities, only if invested, and avoid spamming duplicate orders
            if holding.Invested and symbol.ID.SecurityType == SecurityType.Equity:
                if not self.Transactions.GetOpenOrders(symbol):
                    self.Liquidate(symbol, tag=f"[{self.Time}] OnData flatten equities")

        # drain pending orders that now have prices
        if self.pending_option_orders:
            still_pending = []
        ...
        # --- new dictionary to collect results ---
        ...
        for symbol, earnings_date in self.earnings_calendar.items():
            ...
            for front_contract, back_contract in contract_tuples:
                ...
                if use_cache:
                    ...
                    ivrv_ratio = self.iv_over_rv_ratio(symbol, earnings_date, iv_annualized=slopes['near_iv']) if slopes else None
    ...
    def calculate_iv_slope_pair(self,
        ...
        near_iv, far_iv = self.get_implied_volatilities(front_contract, back_contract, now=self.Time, earnings_time=earnings_time)
